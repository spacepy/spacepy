...
class SeaBase(object):
    ...
    def __init__(self, data, times, epochs, **kwargs):
        ...
        if noncontig:
            ...
            if len(problem_points) > 5:
                print("... and {0} more irregularities".format(
                    len(problem_points)-5))
    ...
    def __str__(self):
        ...
        strtail = 'Data array - {0}; #Epochs - {1}'.format(
            self.data.shape, len(self.epochs))
    ...
    def restoreepochs(self):
        ...
        if self.verbose:
            print('Bad epochs restored to epochs attribute')
    ...
    def _timeepoch(self, delt):
        ...
        if isinstance(el1, dt.datetime) and (type(el1) == type(self.epochs[0])):
            ...
            raise ValueError(
                'Time and Epochs must be consistently typed (numeric/datetime)')
        ...
        if len(lose0) > 0 and len(lose1) > 0:
            ...
            if len(linds) > 0:
                if self.verbose:
                    print(
                        'sea(): {0} out-of-range epochs moved to badepochs attribute'.format(len(linds)))
            if ser_flag:
...
class Sea(SeaBase):
    ...
    def sea(self, **kwargs):
        ...
        nearest_idxs = np.array(
            [np.argmin(np.abs(time - epoch)) for epoch in t_epoch])
        start_idxs = nearest_idxs - wind
        end_idxs = nearest_idxs + wind + 1

        # Process each epoch with optimized slice handling
        for i in range(n):
            st = start_idxs[i]
            en = end_idxs[i]
        ...
        # Compute statistics across all epochs
        self.semean = np.array(
            [np.mean(y_sea_m[:, i].compressed()) for i in range(m)])
        self.semedian = np.array(
        ...
        if kwargs['storedata']:
            ...
            self.timecube = ma.masked_array(t_sea, mask=y_sea_m.mask)
            if self.verbose:
                print('sea(): datacube and timecube added as new attributes')
    ...
    def get_windows(self, *, compress=True):
        """
        Return lists of x-axes (absolute time) and y-values for every event.

        Parameters
        ----------
        compress : bool, default True
            If True, strip the NaN / masked padding so each window is the
            exact length of valid data.  If False, keep rectangular shape
            and return NaNs for padded points.

        Returns
        -------
        xs : list of 1-D numpy.ndarrays
            Absolute time axes in Matplotlib serial-day units. Convert with
            ``matplotlib.dates.num2date`` if you need `datetime` objects.
        ys : list of 1-D numpy.ndarrays
            Corresponding data values.
        """
        if not (hasattr(self, 'datacube') and hasattr(self, 'timecube')):
            raise AttributeError(
                "Run .sea(storedata=True) before get_windows()")

        xs, ys = [], []
        for t_row, y_row in zip(self.timecube, self.datacube):
            if compress:
                xs.append(t_row.compressed())
                ys.append(y_row.compressed())
            else:
                xs.append(t_row.filled(np.nan))
                ys.append(y_row.filled(np.nan))
        return xs, ys
...
class Sea2d(SeaBase):
    ...
    def sea(self, storedata=False, quartiles=True, ci=False, mad=False,
        ...
        # Vectorized calculation of nearest indices
        nearest_idxs = np.array([np.argmin(np.abs(time-epoch))
                                for epoch in t_epoch])

        # Process each epoch with optimized slice handling
        for i in range(n):
            j_idx = nearest_idxs[i]
            st = j_idx - wind
            en = j_idx + wind + 1

            # Handle boundary conditions
            if st < 0:  # Left padding needed
                valid_start = 0
                valid_end = min(en, len(time))
                pad_left = abs(st)
                valid_len = valid_end - valid_start

                if valid_len > 0:
                    y_sea[:, pad_left:pad_left+valid_len,
                          i] = y[:, valid_start:valid_end]
            elif en > len(time):  # Right padding needed
                valid_start = st
                valid_end = len(time)
                valid_len = valid_end - valid_start

                if valid_len > 0:
                    y_sea[:, 0:valid_len, i] = y[:, valid_start:valid_end]
            else:  # Fully inside - no padding needed
                y_sea[:, :, i] = y[:, st:en]
        ...
        # now get SEA quantities - allocate arrays
        self.semean, self.semedian, self.countmask = np.empty(
            (l, m)), np.empty((l, m)), np.empty((l, m))

        #  Vectorized operations using numpy.apply_along_axis:
        self.semean = np.apply_along_axis(lambda x: np.mean(x.compressed())
                                          if len(x.compressed()) > 0 else np.nan,
                                          2, y_sea_m)
        self.semedian = np.apply_along_axis(lambda x: np.median(x.compressed())
                                            if len(x.compressed()) > 0 else np.nan,
                                            2, y_sea_m)
        self.countmask = np.apply_along_axis(
            lambda x: len(x.compressed()), 2, y_sea_m)

        # Apply masking based on count threshold
        self.semean = ma.masked_where(self.countmask < nmask, self.semean)
        self.semedian = ma.masked_where(self.countmask < nmask, self.semedian)
    ...
    def plot(self, xquan='Time Since Epoch', yquan='', xunits='',
        ...
        if zlog:
            cax = ax0.pcolor(self.x, usry, pld, norm=LogNorm(
                vmin=np.nanmin(pld), vmax=np.nanmax(pld)))
        else:
...
def seadict(objlist, namelist):
    ...
    if nobj != nname:
        raise ValueError(
            'seadict(): Lengths of object list and names list must be equal')
    else:
        pass
...
def multisea(dictobj, n_cols=1, epochline=True, usrlimx=[], usrlimy=[],
    ...
    for i in range(n_fig):
        ...
        if pld[i].ndim == 2:  # if semean/median 2D, do colour-plot
            if zlog:
                cax = ax.pcolor(plx[i], ply[i], pld[i], norm=LogNorm(
                    vmin=np.nanmin(pld[i]), vmax=np.nanmax(pld[i])))
            ...
            ax_b = dictobj[keylist[i]].plot(
                xquan='', yquan=ylab[i], epochline=epochline, show=False, target=ax, loc=dum)
...
def sea_signif(obj1, obj2, test='KS', show=True, xquan='Time Since Epoch',
    ...
    show
        (default = True) prints the plot immediately
    xquan
    ...
    results : dm.SpaceData
        SpaceData object containing:
        - x_values: Time axis relative to epoch
        - stat_values: Test statistic values
        - prob_values: P-values at each lag
    ...
    >>> results = seapy.sea_signif(obj1, obj2)
    """
    try:
        ...
        raise TypeError(
            "Inputs must both be seapy.Sea() instances of same length")
    ...
    try:
        ...
        raise NotImplementedError(
            "Test not implemented, only "+str(keylist))
    ...
    ax1 = fig.add_subplot(
        212, position=[pos.bounds[0], 0.1, pos.bounds[2], 0.25])
    ...
    ax1.plot(obj1.x, prob, color='crimson',
             ls='-', lw=1.5, drawstyle='steps-mid')
    ...
    # Make output object to store the test stats and prob
    results = dm.SpaceData()
    results['x_values'] = dm.dmarray(obj1.x, attrs={'units': xunits})
    results['stat_values'] = dm.dmarray(S, attrs={'test': test.upper()})
    results['prob_values'] = dm.dmarray(prob)
    results.attrs['test_name'] = test.upper()
    results.attrs['obj1_name'] = obj1.__class__.__name__
    results.attrs['obj2_name'] = obj2.__class__.__name__
