...
def toHDF5(fname, SDobject, **kwargs):
    ...
    '''
    def SDcarryattrs(SDobject, hfile, path, allowed_attrs, compression=None, compression_opts=None):
        if hasattr(SDobject, 'attrs'):
    ...
    # if we did not specify any compression set that in kwargs
    ...
    for key, value in SDobject.iteritems():
        if isinstance(value, allowed_elems[0]):
            ...
            try:
                hfile[path].create_dataset(key, data=value, compression=kwargs['compression'], compression_opts=kwargs['compression_opts'])
            except:
                dumval = value.copy()
                if isinstance(value[0], datetime.datetime):
                    for i, val in enumerate(value): dumval[i] = val.isoformat()
                hfile[path].create_dataset(key, data=dumval.astype('|S35'), compression=kwargs['compression'], compression_opts=kwargs['compression_opts'])
                #else:
